...
object AnimesProjectFactory : PageParser {
    ...
    private val VERSIONS = Pattern.compile("versoes\\s?=\\s?\\{(.*?)\\};")
    private val VIDEO_URL_PATTERN = Pattern.compile("\"([hml]q)\"\\s?:\\s?\"'?(.*?)'?\"")
    ...
    private fun findLink(iframe: String?): String? {
        ...
        val versionsMatcher = VERSIONS.matcher(html)
        if (!versionsMatcher.find()) return null
        val videoMatcher = VIDEO_URL_PATTERN.matcher(versionsMatcher.group(1))
